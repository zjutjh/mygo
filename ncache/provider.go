...
func Exist(scope string) bool {
	_, err := do.InvokeNamed[Cache](nil, iocPrefix+scope)
	return err == nil
}

// Pick 获取指定scope实例
func Pick(scopes ...string) Cache {
	scope := defaultScope
	if len(scopes) != 0 && scopes[0] != "" {
		scope = scopes[0]
	}
	return do.MustInvokeNamed[Cache](nil, iocPrefix+scope)
}
...
func provide(scope string) error {
	...
	if err != nil {
		return fmt.Errorf("初始化Cache实例错误: %w", err)
	}
